...
message, and link home. Do not optimise placeholder content for search.
...
`Organization`/`LocalBusiness` only when the required facts are supplied. Do not emit
`Service`, `FAQPage`, `BreadcrumbList`, `ContactPage`, review, rating, or other
supporting-page schema as part of this skill.
...
- Route dots have button semantics and accessible labels; expose active state where practical.
...
Use a responsive `<picture>` sized to actual viewport needs. Include dimensions/aspect ratio,
eager loading, async decode, and high fetch priority. Compress visually while preserving the
first scene.
...
picture should win LCP; load clips only near demand.
